...
describe("Deploy a Flash Loan", function () {
  it("Should take a flash loan and be able to return it", async function () {
    ...
    await token
      .connect(signer)
      .transfer(_flashLoanExample.address, BALANCE_AMOUNT_DAI); // Sends our contract 500 DAI from the DAI_WHALE
      console.log(" at line 32");

    const tx = await _flashLoanExample.createFlashLoan(DAI, 1); // Borrow 1 DAI in a Flash Loan with no upfront collateral4
    ...
    expect(remainingBalance <= (BALANCE_AMOUNT_DAI)).to.be.true; // We must have less than 500 DAI now, since the premium was paid from our contract's balance
  });
});